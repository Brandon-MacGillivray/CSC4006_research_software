...
The scope of the present paper is deliberately limited. Although the repository also supports transfer-learning experiments and broader dataset handling, the reported study focuses on the RHD ablation matrix and Jetson Orin Nano deployment benchmarks. The contribution is therefore a baseline RGB 2D study rather than a full stereo-aware HCI system evaluation.

\subsection{DRHand-Inspired Model}

DRHand was selected as the control architecture because it is explicitly designed as a lightweight hand pose estimator and because its dual-regression formulation offers a natural basis for analysing branch complementarity on edge hardware. It is also well matched to the present research question: if some interaction scenarios do not require the full 21-keypoint output, then a compact DRHand-inspired model provides a sensible platform for testing whether reduced landmark layouts can improve efficiency without materially harming useful accuracy.
...
The reported results correspond to the current RGB-only stage of the project. Unless otherwise stated, the accuracy tables summarize five-seed experiments on RHD and the latency tables summarize Jetson Orin Nano benchmarks exported by the repository benchmark pipeline. All values are reported as mean $\pm$ standard deviation. Since only five seeds are available and several differences are small relative to the observed variation, the analysis should be interpreted as preliminary rather than as a final statistical ranking of configurations.
...
Overall, the present findings establish a useful baseline for later stages of the project. They show that the current pipeline is capable of near-real-time operation on embedded hardware while maintaining competitive accuracy within the scope of the implemented experiments. Future work should extend this foundation by improving the fusion mechanism, broadening the evaluation to more varied datasets and application scenarios, and incorporating stereo-aware or depth-informed methods to better address occlusion, viewpoint variation, and robustness in real interactive settings.
