...
Hand pose estimation is an important computer-vision task for touchless
human-computer interaction, especially in edge-oriented settings such as
AR/VR, public displays, and other real-time interactive systems where
accuracy must be balanced against limited compute resources. This project
focuses on lightweight 2D RGB hand pose estimation and implements a
...
The software artefact is structured as a research-oriented repository rather
than an end-user application. It combines a reusable Python package for the
core hand-pose pipeline with command-line scripts, experiment definitions,
preserved checkpoints, and supporting documentation. This report describes the
purpose and operation of the software, its design and implementation, the main
architectural components and interfaces, the data representations it uses, and
the quality-assurance steps taken during development.
...
Dataset substitution. The software assumes that the dual-branch DRHand idea can be studied meaningfully on RHD, HK26K / \texttt{coco\_hand}, and RH8 even though the original paper used YouTube2D Hands, GANerated Hands, STB, and RHD. This means the software evaluates the method under a different data regime, so its results support the reconstructed pipeline used in this project rather than the original paperâ€™s exact benchmark setup.
...
Effective batch-size reconstruction. The software assumes that the paperâ€™s stage-2 batch size of 256 can be approximated using batch size 64 with gradient accumulation on different hardware. This makes the training pipeline usable on more limited systems, but it also means the repository reproduces the paperâ€™s intended effective batch size only approximately rather than literally.
...
This reflects the repository's two-stage optimisation design, in which the
same model is first trained through the heatmap branch and then refined with
joint heatmap and coordinate supervision.
...
This figure is useful because it shows not only which components exist, but
also how control and data move through them during actual use.
...
Together, these interfaces allow the software to move cleanly from raw dataset
input to trained models, reusable checkpoints, and reproducible evaluation
outputs.
...
This common representation allows the same pipeline to operate with either the
standard 21-keypoint layout or the reduced 10-keypoint layout.
...
The interface also relies on explicit file and path arguments for datasets,
...
Finally, the repository shows a consistent testing convention in which small
...
Overall, the automated tests focus on core structural correctness rather than
full end-to-end experiment reproduction, while the latter is handled through
the documented validation and replication commands elsewhere in the
repository.
