...
Pillow. Pillow was used for image loading and resizing because the project processes RGB hand images during training, evaluation, and inference. It was chosen as a simple and dependable library for image I/O and preprocessing.
...
implementation code placed under \path{src/handpose/} and user-facing
...
suite under \path{tests/}. These tests verify important repository contracts
...
\path{tests/test_model_shapes.py}, the test suite checks that
...
\path{tests/test_fusion.py}, the software checks that heatmap argmax
...
\path{tests/test_checkpoints.py}, the suite checks that valid checkpoints
...
Finally, the \path{tests/conftest.py} configuration ensures that the
\path{src/} tree is importable when tests are run from the repository root.
...
part of the core \path{src/handpose/} repository implementation described in
...
primarily through hand movement and fist-based selection, so the system can be
used during presentation without relying on a conventional mouse or keyboard
even though the underlying Qt widgets still retain their normal click
behaviour.
...
this gives the demonstrator two forms of interaction: continuous hand motion is
...
The demonstrator can be understood as a small interactive architecture with
three main parts, as shown in Figure~\ref{fig:interactive-demo-architecture}.
First, a perception pipeline acquires live frames from the ZED camera, rotates
and preprocesses them, and applies MediaPipe Hands to estimate landmarks from
which cursor position, palm width, and fist-click state are derived. Second, a
central \texttt{MainWindow} acts as the controller for the application: it
...
implemented as a stacked set of pages for the start screen, menu,
...
\caption{Architecture of the interactive demonstrator, showing the live hand-tracking pipeline, central window controller, and gesture-driven demo pages.}
\label{fig:interactive-demo-architecture}
\end{figure}

\medskip

This structure is an important design choice because it separates the
latency-sensitive perception path from the presentation layer while still
allowing both demonstrations to share the same interaction controller.
Continuous hand motion drives cursor movement and the tone-mapping spotlight,
whereas fist closures are treated as click events for page
navigation and segmentation selection. Camera acquisition and hand-tracking
would make the interface feel unresponsive if they were executed directly in
the GUI event loop, so the vision thread emits pose updates to the main window
through Qt signals while a timer refreshes the overlay at a steady rate. 
This reduces the risk of the interface freezing
during active use and makes the flow of interaction state easier to reason
about.
...
The data handling for these demonstrations is therefore simple but explicit. In
the single-mask segmentation mode, the software loads one base microscopy image
and one binary mask, computes connected components from the foreground pixels,
and stores pre-rendered coloured overlays for each selectable region. In the
RNvU mode, the software instead loads a base image together with several
binary class masks and combines them into an integer label map so that a click
can be resolved to a specific tissue class. The tone-mapping view uses fixed
pairs of baseline and improved images, and the size of the spotlight is
derived from the tracked palm width so that the reveal area scales with the
user's hand.

\subsection{Inputs, Outputs, Dependencies, and Limitations}

The main runtime inputs combine live camera frames from the ZED device with
local image, mask, and branding assets prepared for the demonstration.
