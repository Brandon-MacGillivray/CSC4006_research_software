...
\textbf{Evidence:} In Table~\ref{tab:transfer_learning}, the zero-shot cross-dataset runs are weak: \texttt{C\_ONLY} on RHD gives SSE 1.4194, EPE 0.1609, and PCK 0.3397, while \texttt{R\_ONLY} on COCO gives SSE 3.7488, EPE 0.3365, and PCK 0.3719. After target-domain fine-tuning, the transfer runs improve sharply: \texttt{C\_TO\_R} reaches SSE 0.4504, EPE 0.1020, and PCK 0.8190 on RHD, while \texttt{R\_TO\_C} reaches SSE 0.0980, EPE 0.0840, and PCK 0.9837 on COCO.
\textbf{Explanation:} This suggests that the source-domain model does learn reusable hand-pose structure, but that knowledge is not well calibrated to a new dataset on its own. Fine-tuning helps because it retains the useful hand representation from pretraining while adapting the model to the target domain's appearance, pose distribution, and annotation characteristics.
...
\textbf{Evidence:} Table~\ref{tab:transfer_learning} shows that \texttt{R\_TO\_C} ends very close to COCO scratch performance: COCO scratch gives SSE 0.0764 and PCK 0.9834, while \texttt{R\_TO\_C} gives SSE 0.0980 and PCK 0.9837. In contrast, \texttt{C\_TO\_R} remains noticeably behind RHD scratch performance: RHD scratch gives SSE 0.3091 and PCK 0.9143, while \texttt{C\_TO\_R} gives SSE 0.4504 and PCK 0.8190.
\textbf{Explanation:} This indicates that transfer difficulty depends on the direction of adaptation, not just on whether transfer is used. In this study, RHD pretraining appears to provide a stronger starting point for later COCO adaptation than COCO pretraining provides for later RHD adaptation, likely reflecting differences in domain mismatch between the two datasets. The same asymmetry is also visible qualitatively: \texttt{R\_TO\_C} tracks the manually annotated real-image set much better than \texttt{C\_TO\_R}, whereas \texttt{C\_ONLY} fails visibly on selected RHD examples.
\textbf{Link:} Accordingly, the transfer study shows not only that fine-tuning helps, but also that some directions of adaptation are easier than others, both quantitatively and qualitatively.

\medskip
\noindent\textbf{Point:} The learned DRHand-style models outperform the MediaPipe baseline on the benchmark RHD evaluation, although MediaPipe remains visually strong on the small manually annotated real-image set.
\textbf{Evidence:} In Table~\ref{tab:mediapipe_baseline}, MediaPipe evaluated with \texttt{--hand auto} still gives much worse SSE and PCK than the learned models in both native-21 and shared-10 comparisons on RHD, despite achieving a detection success rate of 0.8933. However, the qualitative figures on the manually annotated real-image set show that MediaPipe often aligns closely with the corrected labels.
\textbf{Explanation:} The benchmark result is the more reliable comparison because the small real-image set was initialized from MediaPipe predictions before manual correction, which gives MediaPipe a structural advantage on that qualitative case-study set.
\textbf{Link:} MediaPipe is therefore useful as an external reference, but the main benchmark evidence still favors the learned pipeline.
...
\noindent\textbf{Point:} A low oracle-match rate does not mean that fusion is ineffective.
\textbf{Evidence:} The fusion diagnostics show oracle values of only around 0.48 to 0.53, yet the branch-ablation results still show that fusion gives the strongest overall balance of SSE and EPE compared with either individual branch.
\textbf{Explanation:} The oracle metric measures how often the heuristic selects the same branch as the lower-error branch on a per-joint basis, whereas SSE and EPE measure the magnitude of the final prediction errors. As a result, fusion can still improve overall accuracy if it makes correct choices on the relatively small number of high-impact joints where the two branches differ substantially, while many oracle mismatches may occur on low-impact or near-tied joints that contribute little to the final error.
\textbf{Link:} Therefore, the current results suggest that fusion is beneficial overall, even though the hand-crafted branch-selection rule remains far from optimal.



\subsection{Qualitative Results}

\noindent\textbf{Point:} The manually annotated real-image set behaves more like the COCO-style domain than the RHD domain.
\textbf{Evidence:} On the eight-image \texttt{hand\_frame\_small} set, \texttt{R\_TO\_C} and \texttt{C\_ONLY} are the strongest learned-model families, with PCK values of 0.9425 and 0.9200 respectively, whereas \texttt{B0}, \texttt{B1}, \texttt{R\_ONLY}, and \texttt{C\_TO\_R} remain in the range 0.6475 to 0.7075. The corresponding qualitative figures also show that the COCO-oriented models usually place the reduced keypoints closest to the manually annotated ground truth.
\textbf{Explanation:} Although the real-image set is very small, its appearance and pose characteristics are evidently closer to the COCO-style hand data than to the synthetic RHD domain.
\textbf{Link:} This qualitative result is consistent with the transfer-learning results and helps explain why COCO-trained models generalize better on this small real-image set.

\medskip
\noindent\textbf{Point:} The RHD qualitative figures confirm that zero-shot cross-domain transfer from COCO to RHD is weak.
\textbf{Evidence:} In several RHD examples, such as 00143 and 00477, the \texttt{C\_ONLY} model places multiple predicted joints away from the hand, whereas the RHD-trained models and the transferred models remain much closer to the annotated finger structure.
\textbf{Explanation:} This is the visual counterpart of the quantitative transfer table, where \texttt{C\_ONLY} performs poorly on RHD and target-domain fine-tuning substantially improves performance.
\textbf{Link:} The qualitative evidence therefore strengthens the conclusion that transfer is useful mainly as initialization for later target-domain adaptation.

\medskip
\noindent\textbf{Point:} MediaPipe should be treated as an illustrative qualitative baseline rather than a fully independent comparator on the manually annotated real-image set.
\textbf{Evidence:} MediaPipe is visually very strong on the small real-image set and also achieves perfect scores in the derived summary, but the manual annotations were initialized from MediaPipe predictions before correction.
\textbf{Explanation:} This creates a structural advantage for MediaPipe on that set, even though it remains a useful reference for visual comparison. By contrast, on the benchmark RHD evaluation the learned DRHand-style models still outperform MediaPipe by a large margin.
\textbf{Link:} The small real-image set is therefore best interpreted as a qualitative case study rather than a standalone benchmark for baseline ranking.
...
Future work should focus on improving the fusion stage, since the current diagnostics show that the heuristic often fails to match the oracle branch choice. In addition to improving the selection rule itself, a useful extension would be to measure not only how often the heuristic disagrees with oracle, but also how costly those mistakes are. This could be done by reporting an oracle-regret metric, defined as the extra error incurred by the heuristic relative to the lower-error branch for each visible joint. Such a measure would help distinguish between harmless near-tie mismatches and genuinely costly branch-selection failures. The qualitative comparisons further suggest that the small manually annotated real-image set is more COCO-like than RHD-like, reinforcing the observed transfer asymmetry and the importance of domain mismatch in practical deployment.
