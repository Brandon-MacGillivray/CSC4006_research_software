...
\medskip
\noindent\textbf{Point:} Existing lightweight methods show that real-time hand tracking is feasible, but they leave open design questions about representation, post processing techniques, and generalisation with limited training data.
...
\textbf{Explanation:} These methods establish a strong baseline, but certain questions emerge, such as do we need all 21 keypoints? is the fusion used in DRHand actually better than single branch inference or unnecessary computation? And in light of limited available datasets, what can be done to improve generalisation? 
...
\textbf{Evidence:} The study compares 21-keypoint and reduced 10-keypoint prediction, evaluates fusion against the individual branches, analyses transfer between RHD and COCO-style annotated pseudo-label dataset generated using Mediapipe, compares against a MediaPipe baseline, and benchmarks runtime on NVIDIA Jetson orin nano hardware.
...
\textbf{Explanation:} This suggests that efficient pose estimation remains an open design problem.  
...
\textbf{Evidence:} Prior work such as SPLite Hand~\cite{splitehand} mentions the tradeoff and actively caters for it. while other papers assume desktop level compute [TODO]  
...
\medskip
\noindent\textbf{Point:} The choice of training data is known to affect model generalisation.  
...
\textbf{Explanation:} The input is a resized RGB hand image, and most computation is shared before the network splits into heatmap and coordinate regression paths. The same structure is used for both the standard 21-keypoint model and the reduced 10-keypoint variant, with only the output dimensionality changing.

Formally, the input to the model is an RGB image resized to $256 \times 256$, denoted by $x \in \mathbb{R}^{3 \times 256 \times 256}$. The backbone then produces a compact feature representation,
\[
F = B(x).
\]
...
\textbf{Explanation:} The backbone is built from depthwise-separable convolutions, downsampling blocks, and skip-connected upsampling stages.
...
\]
where \(K\) is the number of predicted keypoints. For keypoint \(i\), \(\hat{H}_i(u,v)\) denotes the predicted confidence at spatial location \((u,v)\) on the \(64 \times 64\) heatmap grid, where \(u\) and \(v\) are the horizontal and vertical heatmap indices. Let \(p_i = (x_i, y_i)\) denote the ground-truth location of keypoint \(i\), expressed in the same heatmap coordinate system. The target heatmap for keypoint \(i\) is then defined as a 2D Gaussian centered at the true keypoint location,
\[
G_i(u,v) = \exp\left(
-\frac{(u-x_i)^2 + (v-y_i)^2}{2\sigma^2}
\right),
\]
where \(\sigma\) controls the spread of the Gaussian. A larger \(\sigma\) produces a broader peak, while a smaller \(\sigma\) produces a sharper target concentrated around the true landmark.

The heatmap branch is supervised using mean-squared error between the predicted heatmaps and the Gaussian target heatmaps:
\[
\mathcal{L}_{hm}
=
\frac{1}{KHW}
...
\]
where \(F_{64}\) denotes the intermediate feature map passed from the heatmap branch to the coordinate regressor, \(R_c(\cdot)\) denotes the coordinate-regression subnetwork, and \(K\) is the number of predicted keypoints. For keypoint \(i\), the output \(\hat{p}^{coord}_i = (\hat{x}_i, \hat{y}_i)\) represents the predicted normalised 2D coordinate, with both values expressed in the range \([0,1]\).
...
\textbf{Explanation:} Unless otherwise stated, results for the learned DRHand variants are summarised over five random seeds and reported as mean $\pm$ standard deviation. This reduces the likelihood that conclusions depend on a single training run.
...
\medskip
\noindent\textbf{Point:} All model inputs are normalised RGB images resized to a fixed spatial resolution.  
...
\textbf{Explanation:} In stage 1, the backbone and heatmap branch are optimised first. In stage 2, the coordinate branch is trained together with optional fine-tuning of the rest of the model. This preserves the staged training logic of the original DRHand design.

\medskip
\noindent\textbf{Point:} Transfer-learning runs are implemented through checkpoint initialisation and target-domain fine-tuning.  
\textbf{Explanation:} For \texttt{C\_TO\_R} and \texttt{R\_TO\_C}, training begins from a source-domain checkpoint and continues on the target dataset.

\medskip
\noindent\textbf{Point:} Training and evaluation were performed on high-performance GPU hardware.  
\textbf{Explanation:} The learned models were trained and evaluated on an NVIDIA Tesla A100 80GB GPU.
...
\medskip
\noindent\textbf{Point:} Embedded deployment performance was measured separately.  
...
\textbf{Explanation:} EPE is computed after subtracting a designated root joint from both the prediction and the ground truth. This removes global translation and measures how well the relative hand structure is recovered. Let \(r\) denote the root keypoint used for the current evaluation layout. In the full layout, keypoint \(0\) is used when available. In reduced or shared-10 evaluation, keypoint \(1\) is used when keypoint \(0\) is absent.
...
\textbf{Explanation:} For the MediaPipe baseline, detection success rate is reported because the pipeline may fail before landmark prediction. For fusion analysis, the study also reports how often each branch is selected, how strongly the two branches disagree, the typical size of the adaptive fusion threshold, and how often the heuristic agrees with an oracle branch choice computed using ground truth.
